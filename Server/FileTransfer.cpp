...
// Liên kết thư viện mã hóa của Windows (Có sẵn, không cần cài thêm)
#pragma comment(lib, "Crypt32.lib")

// =============================================================
// PHẦN 1: CÁC HÀM HỖ TRỢ CHUYỂN ĐỔI (UNICODE HELPER)
// =============================================================

// Chuyển từ std::string (UTF-8) sang std::wstring (UTF-16/Unicode)
// Mục đích: Dùng để gọi các API của Windows (như FindFirstFileW, CreateFileW...)
...
// Mục đích: Dùng để gửi dữ liệu qua Socket (Giao thức mạng thường dùng UTF-8)
...
// =============================================================
// PHẦN 2: LOGIC TẢI FILE (DOWNLOAD)
// =============================================================
...
std::string FileTransfer::HandleDownloadRequest(const std::string& filepath) {
    ...
    // 2. Mở file ở chế độ Binary (Quan trọng để không làm hỏng dữ liệu ảnh/exe)
    ...
    DWORD outLen = 0;
    // Bước 5a: Lấy kích thước buffer cần thiết cho chuỗi Base64
    ...
    // Lấy tên file gốc từ đường dẫn đầy đủ để Client lưu đúng tên
...
// =============================================================
// PHẦN 3: LOGIC DUYỆT THƯ MỤC (DIRECTORY LISTING)
// =============================================================
...
std::string FileTransfer::ListDirectory(const std::string& path) {
    ...
    if (path == "MyComputer" || path.empty()) {
        ...
        // Lấy danh sách ổ đĩa logic (Vd: "C:\0D:\0\0")
        char buffer[256];
    ...
    // 2. LIỆT KÊ FILE TRONG THƯ MỤC CỤ THỂ
    std::string searchPath = path;

    // Fix lỗi: Nếu người dùng gửi "C:" (thiếu backslash) -> thêm thành "C:\" để Windows hiểu
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        // Nếu lỗi (không quyền truy cập, sai đường dẫn), trả về danh sách ổ đĩa để fallback
